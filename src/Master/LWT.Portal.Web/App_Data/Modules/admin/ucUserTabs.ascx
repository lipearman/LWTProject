<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ucUserTabs.ascx.vb" Inherits="LWT.Portal.Web.ucUserTabs" %>
    ...
    SelectCommand=" 
     ...
     ( 
       ...
       select PortalId*-1 as TabId
        ,'Project: ' + PortalName as TabName
        , 0 as OrderBy
        , null as  ParentId
        , PortalId
        FROM PortalCfg_Globals
        where PortalId in(
		      select distinct PortalCfg_Tabs.PortalId 
		      from Portal_Users
		      inner join Portal_UserRoles on [Portal_UserRoles].UserID = Portal_Users.UserID  
		      inner join Portal_TabRoles on [Portal_UserRoles].RoleID = Portal_TabRoles.RoleID
		      inner join dbo.PortalCfg_Tabs on PortalCfg_Tabs.TabId = Portal_TabRoles.TabId
		      where Portal_Users.UserName=@UserName 
        )    
        ...
        union all
            ...
            inner join dbo.PortalCfg_Tabs on PortalCfg_Tabs.TabId = Portal_TabRoles.TabId
            where Portal_TabRoles.RoleID in
        (
            select RoleId from Portal_Users
            inner join Portal_UserRoles 
            on [Portal_UserRoles].UserID = Portal_Users.UserID  
