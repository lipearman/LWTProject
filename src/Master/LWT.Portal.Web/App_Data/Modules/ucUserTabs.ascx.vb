...
Public Class ucUserTabs
    ...
    Protected Sub callbackPanel_view_Callback(ByVal source As Object, ByVal e As DevExpress.Web.CallbackEventArgsBase) Handles callbackPanel_view.Callback
        ...
        '    Dim _node = (From c In dc.Portal_UserTabs Where c.USERID.Equals(_user.UserID)).ToList()
    ...
    Protected Sub cbSave_Callback(ByVal sender As Object, ByVal e As CallbackEventArgs) Handles cbSave.Callback
        ...
        Using dc As New DataClasses_PortalDataContextExt()
            Dim _user = (From c In dc.Portal_Users Where c.UserName.Equals(_username)).FirstOrDefault()
            Dim _Tabs = (From c In dc.PortalCfg_Tabs).ToList()
            For Each item In _SelectedNodes
            ...
            If _selectedTabIds.Count > 0 Then
                Dim _Portals = _Tabs.Select(Function(c) c.PortalId).Distinct.ToList()
                Dim _tabids = _selectedTabIds.Distinct().ToList()

                '=================== Delete TabId ==================
                Dim _data_del = (From c In dc.Portal_UserTabs Where c.USERID.Equals(_user.UserID) And _Portals.Contains(c.PortalId) And Not _tabids.Contains(c.TABID)).ToList()
                If _data_del.Count > 0 Then
                    dc.Portal_UserTabs.DeleteAllOnSubmit(_data_del)
                End If
                ...
                For Each TabID In _tabids
                    Dim _data = (From c In dc.Portal_UserTabs Where c.USERID.Equals(_user.UserID) And _Portals.Contains(c.PortalId) And c.TABID.Equals(TabID)).FirstOrDefault()
                    If _data Is Nothing Then
                        Dim _Portal = (From c In dc.PortalCfg_Tabs Where c.TabId.Equals(TabID)).FirstOrDefault()

                        _newnode.Add(New Portal_UserTab With {.TABID = _Portal.TabId, .USERID = _user.UserID, .PortalId = _Portal.PortalId})
                    End If
                Next
                If _newnode.Count > 0 Then
                    dc.Portal_UserTabs.InsertAllOnSubmit(_newnode)
