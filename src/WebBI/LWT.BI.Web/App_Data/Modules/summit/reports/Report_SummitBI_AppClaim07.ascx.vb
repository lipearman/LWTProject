...
Partial Class Reports_Report_SummitBI_AppClaim07
    ...
    '    'SqlDataSource_gridData.SelectCommand = String.Format("SELECT * FROM RawData_SummitBI_AppClaim07 where convert(varchar,[AddedDate],112) between '{0}' and '{1}' ", DirectCast(DateFrom.Value, Date).ToString("yyyyMMdd"), DirectCast(DateTo.Value, Date).ToString("yyyyMMdd"))
    ...
    '        Case "AddedDate"
    '            Dim op1 = New BinaryOperator("AddedDate", DateFrom.Value, BinaryOperatorType.GreaterOrEqual)
    '            Dim op2 = New BinaryOperator("AddedDate", DateAdd(DateInterval.Day, 1, DateTo.Value), BinaryOperatorType.Less)
    '            Dim op = New GroupOperator(GroupOperatorType.And, New CriteriaOperator() {op1, op2})
    '            gridData.FilterExpression = op.ToString()

    '        Case "DateReceivedDocFromClient"
    '            Dim op1 = New BinaryOperator("DateReceivedDocFromClient", DateFrom.Value, BinaryOperatorType.GreaterOrEqual)
    '            Dim op2 = New BinaryOperator("DateReceivedDocFromClient", DateAdd(DateInterval.Day, 1, DateTo.Value), BinaryOperatorType.Less)
    '            Dim op = New GroupOperator(GroupOperatorType.And, New CriteriaOperator() {op1, op2})
    '            gridData.FilterExpression = op.ToString()
    ...
    Protected Sub SqlDataSource_Selecting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceSelectingEventArgs) Handles SqlDataSource_gridData.Selecting
        ...
        If DateFrom.Value IsNot Nothing And DateTo.Value IsNot Nothing Then
            ...
            Select Case FilterDateType.SelectedItem.Value
                ...
                Case "AddedDate"
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where convert(varchar,AddedDate,112) between '" & _DateFrom.ToString("yyyyMMdd") & "' and '" & _DateTo.ToString("yyyyMMdd") & "'"


                Case "DateReceivedDocFromClient"
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where convert(varchar,DateReceivedDocFromClient,112) between '" & _DateFrom.ToString("yyyyMMdd") & "' and '" & _DateTo.ToString("yyyyMMdd") & "'"


                Case "DateMakeContract"
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where convert(varchar,DateMakeContract,112) between '" & _DateFrom.ToString("yyyyMMdd") & "' and '" & _DateTo.ToString("yyyyMMdd") & "'"
                Case "DateMakePolicy"
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where convert(varchar,DateMakePolicy,112) between '" & _DateFrom.ToString("yyyyMMdd") & "' and '" & _DateTo.ToString("yyyyMMdd") & "'"
                Case "DateBegin"
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where convert(varchar,DateBegin,112) between '" & _DateFrom.ToString("yyyyMMdd") & "' and '" & _DateTo.ToString("yyyyMMdd") & "'"
                Case "DateEnd"
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where convert(varchar,DateEnd,112) between '" & _DateFrom.ToString("yyyyMMdd") & "' and '" & _DateTo.ToString("yyyyMMdd") & "'"
                Case Else
                    e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where DateMakeContract is null "
            End Select
        Else
            e.Command.CommandText = "select * from RawData_SummitBI_AppClaim07 where DateMakeContract is null "
        End If
