...
Partial Class applications_BI_Master_Preview
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Init
        If (Not IsPostBack) Then
            ...
            If Session("BID") IsNot Nothing Then

                Dim _BID = CInt(Session("BID"))
    ...
    Private Sub BindData2(ByVal _BID As Integer)
        Using dc As New DataClasses_PortalBIExt()
            pivotGrid.Caption = ""
            pivotGrid.Fields.Clear()
        ...
        Using dc As New DataClasses_PortalBIExt()
            For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                If _field.Visible = True Then
                    Dim FilterValues = (From c In dc.tblBIFilters Where c.FIELD.Equals(_field.FieldName) And c.BID.Equals(_BID)).ToList()
    ...
    Private Sub BindSort(ByVal _BID As Integer)
        Using dc As New DataClasses_PortalBIExt()


            For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                'If _field.Visible = True Then
                Dim _Attributes = (From c In dc.tblBIAttributes Where c.FIELD.Equals(_field.FieldName) And c.BID.Equals(_BID)).FirstOrDefault()
    ...
    Private Sub BindSortBySummary(ByVal _BID As Integer)
        Using dc As New DataClasses_PortalBIExt()
            Dim data = (From c In dc.tblBIAttributes Where c.SortBySummaryInfo <> "" And c.BID = _BID).ToList()
