...
Partial Class Modules_ucDevxBIFileLWTBIUser
        ...
        If e.Column.FieldName = "BID" Then
            ...
            Using dc As New DataClasses_PortalBIExt()
                ...
                If _rpt.DS_ID Is Nothing Then
                    ...
                    '    e.Cell.Text = "<a class=""dxeHyperlink_Office2010Blue"" href=""javascript:ShowMasterBIPopup('" & _ds.GUID & "');"">" & _rpt.TITLE & "</a>"
                End If
            End Using
        End If
    ...
    Protected Sub SqlDataSource1_Selecting(ByVal sender As Object, ByVal e As SqlDataSourceSelectingEventArgs) Handles SqlDataSource_Data.Selecting
        ...
        Using dc As New DataClasses_PortalBIExt()
            Dim _data = (From c In dc.V_DataSourceBI_Assignments Where c.USERNAME.Equals(HttpContext.Current.User.Identity.Name) And c.PortalId = portalSettings.PortalId Order By c.ParentId, c.No).ToList()


            Dim _Reports = (From c In dc.tblDataSourceBIs Where c.PortalId = portalSettings.PortalId Order By c.ParentID, c.No).ToList()
            ...
            For Each item In _data
                ...
                If _Child IsNot Nothing Then
                    If _Child.ParentID IsNot Nothing Then
                        Dim _Parent = (From c In _Reports Where c.BID = _Child.ParentID Order By c.ParentID, c.No).FirstOrDefault()
            ...
            If _BID.Count > 0 Then
                ...
                e.Command.CommandText = String.Format("select * from tblDataSourceBI where BID in({0}) order by ParentID, No", sb.ToString())
