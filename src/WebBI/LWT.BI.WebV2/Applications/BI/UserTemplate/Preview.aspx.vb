...
Partial Class applications_BI_UserTemplate_Preview
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Init
        If (Not IsPostBack) Then
            ...
            If Session("BUID") IsNot Nothing Then
                Try
                    Dim _BUID = CInt(Session("BUID"))
    ...
    Private Sub BindData(ByVal _BUID As Integer)
        Using dc As New DataClasses_PortalBIExt()
            pivotGrid.Caption = ""
            pivotGrid.Fields.Clear()

            Dim _BI = (From c In dc.tblBI_Users Where c.BUID.Equals(_BUID)).FirstOrDefault()
            ...
            'Dim _Attributes = (From c In dc.V_Attributes Where c.CUBE_ID.Equals(_BI.CUBE_ID)).ToList()
            Dim _Attributes = (From c In dc.V_BIAttribute_Users Where c.BUID.Equals(_BI.BUID) Order By c.AREA, c.AREAINDEX).ToList()
            ...
            If _Attributes.Count > 0 Then
                ...
                For Each _Attribute In _Attributes.Where(Function(c) c.Type.Equals("Measure") Or c.Type.Equals("Attribute")).ToList()
                    ...
                    With _NewField
                        ...
                        ''======================= Area =========================
                        'Dim _BIAttribute_Users = (From c In dc.tblBIAttribute_Users Where c.BUID.Equals(_BUID) And c.FIELD.Equals(_Attribute.FIELD)).FirstOrDefault()
                        'If _BIAttribute_Users IsNot Nothing Then
                        '    If _BIAttribute_Users.AREA < 0 Then
                        '        .Visible = False
                        '    Else
                        '        .Visible = True
                        '        .Area = _BIAttribute_Users.AREA


                        '    End If
                        'Else
                        '    .Visible = False
                        '    '.AreaIndex = _Attribute.ATTRIBUTE
                        'End If
    ...
    Private Sub BindData2(ByVal _BUID As Integer)
        Using dc As New DataClasses_PortalBIExt()
            pivotGrid.Caption = ""
            pivotGrid.Fields.Clear()
            ...
            For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                ...
                '======================= Area =========================
                Dim _Attribute = (From c In dc.V_BIAttribute_Users Where c.BUID.Equals(_BI.BUID) And c.FIELD.Equals(_field.FieldName) Order By c.AREA, c.AREAINDEX).FirstOrDefault()
    ...
    Private Sub BindFilter(ByVal _BUID As Integer)
        Using dc As New DataClasses_PortalBIExt()
            For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                If _field.Visible = True Then
                    Dim FilterValues = (From c In dc.tblBIFilter_Users Where c.FIELD.Equals(_field.FieldName) And c.BUID.Equals(_BUID)).ToList()
    ...
    Private Sub BindSort(ByVal _BUID As Integer)
        Using dc As New DataClasses_PortalBIExt()


            For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                If _field.Visible = True Then
                    Dim _Attributes = (From c In dc.tblBIAttribute_Users Where c.FIELD.Equals(_field.FieldName) And c.BUID.Equals(_BUID)).FirstOrDefault()
    ...
    Private Sub BindSortBySummary(ByVal _BUID As Integer)
        Using dc As New DataClasses_PortalBIExt()
            Dim data = (From c In dc.tblBIAttribute_Users Where c.SortBySummaryInfo <> "" And c.BUID = _BUID).ToList()
    ...
    Protected Sub cbSaveBI_Callback(ByVal sender As Object, ByVal e As CallbackEventArgs) Handles cbSaveBI.Callback
        Try
            Using dc As New DataClasses_PortalBIExt()
                ...
                '================================ Delete Filter =========================
                Dim _BIFilter = (From c In dc.tblBIFilter_Users Where c.BUID.Equals(_BUID)).ToList()
                dc.tblBIFilter_Users.DeleteAllOnSubmit(_BIFilter)
                dc.SubmitChanges()

                '================================ Delete tblBIAttribute_Users =========================
                Dim _BIAttribute = (From c In dc.tblBIAttribute_Users Where c.BUID.Equals(_BUID)).ToList()
                dc.tblBIAttribute_Users.DeleteAllOnSubmit(_BIAttribute)
                dc.SubmitChanges()


                '================================ Add tblBIAttribute_Users =========================
                Dim _newfields As New List(Of tblBIAttribute_User)
                For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                    If _field.Visible = True Then
                        Dim _BI_Field = (From c In dc.tblBIAttribute_Users Where c.BUID.Equals(_BUID) And c.FIELD.Equals(_field.FieldName)).FirstOrDefault()
                        ...
                        Else
                            Dim _newfield As New tblBIAttribute_User
                            _newfield.BUID = _BUID
                    ...
                    ElseIf _field.Visible = False Then
                        Dim _BI_Field = (From c In dc.tblBIAttribute_Users Where c.BUID.Equals(_BUID) And c.FIELD.Equals(_field.FieldName)).FirstOrDefault()
                        If _BI_Field IsNot Nothing Then
                            dc.tblBIAttribute_Users.DeleteOnSubmit(_BI_Field)
                            dc.SubmitChanges()
                        End If
                    End If
                Next
                ...
                If _newfields.Count > 0 Then
                    dc.tblBIAttribute_Users.InsertAllOnSubmit(_newfields)
                    dc.SubmitChanges()
                End If
                ...
                '================================ Add Filter =========================
                Dim _NewFilters As New List(Of tblBIFilter_User)
                For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                    If _field.Visible = True Then
                        If _field.FilterValues.HasFilter Then
                            For Each itemvalue In _field.FilterValues.ValuesIncluded
                                _NewFilters.Add(New tblBIFilter_User With {.BUID = _BUID, .FIELD = _field.FieldName, .FILTER_TYPE = itemvalue.GetType().ToString(), .FILTER_VALUE = itemvalue.ToString()})
                            Next
                        End If
                    End If
                Next
                dc.tblBIFilter_Users.InsertAllOnSubmit(_NewFilters)
                dc.SubmitChanges()


 

                'Dim _BIAttribute = (From c In dc.tblBIFilter_Users Where c.BUID.Equals(_BUID)).ToList()
                'dc.tblBIFilter_Users.DeleteAllOnSubmit(_BIAttribute)


                'Dim _BIAttributes As New List(Of tblBIAttribute_User)

                'For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                '    If _field.Visible = True Then
                '        Dim _Attribute As New tblBIAttribute_User

                '        _Attribute.FIELD = _field.FieldName
                '        _Attribute.BUID = _BUID
                ...
                '            For Each itemvalue In _field.FilterValues.ValuesIncluded
                '                _Attribute.tblBIFilter_Users.Add(New tblBIFilter_User With {.FIELD = _Attribute.FIELD, .BUID = _Attribute.BUID, .FILTER_TYPE = itemvalue.GetType().ToString(), .FILTER_VALUE = itemvalue.ToString()})
                '            Next
                ...
                'dc.tblBIAttribute_Users.InsertAllOnSubmit(_BIAttributes)
                'dc.SubmitChanges()
