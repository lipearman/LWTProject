...
Partial Class applications_BI_Master_PreviewExcel
    ...
    Private Sub BindSortBySummary()
        Using dc As New DataClasses_PortalBIExt()
            Dim data = (From c In dc.tblDataSourceBI_Fields Where c.SortBySummaryInfo IsNot Nothing And c.BID = hdBID.Value).ToList()
    ...
    Private Sub BindFilter()
        ...
        Using dc As New DataClasses_PortalBIExt()
            For Each _field As DevExpress.Web.ASPxPivotGrid.PivotGridField In pivotGrid.Fields
                If _field.Visible = True Then
                    Dim FilterValues = (From c In dc.tblDataSourceBI_Field_Filters Where c.FIELD_NAME.Equals(_field.FieldName) And c.BID.Equals(hdBID.Value)).ToList()
