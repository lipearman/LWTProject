...
Partial Class Modules_ucDevxNLTHGetTempData
    ...
    Private Sub binddata()
        ...
        '============ todo2=======================
        SqlDataSource_gridData.SelectCommand = String.Format("SELECT ROW_NUMBER() OVER(ORDER BY DateNotification) AS [RowNo],* FROM V_NLTH_GetTemp_Daily where  convert(varchar,DateNotification,112) between '{0}' and '{1}' ", DirectCast(DateFrom.Value, Date).ToString("yyyyMMdd"), DirectCast(DateTo.Value, Date).ToString("yyyyMMdd"))
