...
Partial Class Modules_ucDevxNoticeMailContactSetup_B0022
    Inherits PortalModuleControl
    Private NoticeCode As String = "B0022"
    ...
    Protected Sub frmImport_Upload_FileUploadComplete(ByVal sender As Object, ByVal e As DevExpress.Web.FileUploadCompleteEventArgs) Handles frmImport_UploadFile.FileUploadComplete
        ...
        If frmImport_UploadFile.Page IsNot Nothing Then
            ...
            Dim constr = ConfigurationManager.ConnectionStrings("CPSConnectionString").ConnectionString

            'Dim InsertedTableName = "tblAccountingDNRemark"
            'Dim dm As New Datamanage
            'Try

            Dim dt1 = MyUtils.ExcelToDataTable(FileName)
 
            'Try
            Dim dt2 As New DataTable()
            dt2.Columns.AddRange(New DataColumn(5) {New DataColumn("NoticeCode", GetType(String)), _
                                                   New DataColumn("Code", GetType(String)), _
                                                   New DataColumn("Name", GetType(String)), _
                                                   New DataColumn("MailTo", GetType(String)), _
                                                   New DataColumn("MailCC", GetType(String)), _
                                                   New DataColumn("ContactName", GetType(String))})

            For Each row As DataRow In dt1.Rows
                If Not DBNull.Value.Equals(row("รหัส")) Then
                  
                    Try
                        Dim _NoticeCode As String = NoticeCode
                        Dim _Code As String = row("รหัส")
                        Dim _Name As String = row("ตัวแทน")
                        Dim _MailTo As String = row("To")
                        Dim _MailCC As String = IIf(DBNull.Value.Equals(row("CC")), "", row("CC"))
                        Dim _ContactName As String = row("ชื่อผู้ติดต่อ")


                        dt2.Rows.Add(_NoticeCode, _Code, _Name, _MailTo, _MailCC, _ContactName)
                    Catch ex As Exception
                        Throw New Exception(String.Format("Error: {0} - {1}", row("รหัส"), row("ตัวแทน")))
                    End Try
            ...
            Using con As New SqlConnection(constr)
                Using cmd As New SqlCommand("Update_NoticeMailContact")
                    cmd.CommandType = CommandType.StoredProcedure
                    cmd.Connection = con
                    cmd.Parameters.AddWithValue("@NoticeMailContact", dt2)
                    con.Open()
                    cmd.ExecuteNonQuery()
                    con.Close()
                End Using
