...
Partial Class Modules_ucDevxNoticeHeaderSetup_B0016N
    Inherits PortalModuleControl
    Dim _MailNotifications_Amend As String = "B0015N"
    'Dim _MailNotifications_Endorse As String = "B0016"

    Dim _NoticeCode As String = "B0016N"
    ...
    Protected Sub btnDownloadFormat_click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Dim filePath = String.Format("{0}\{1}", Server.MapPath("~/Template"), "Template for Endorsement send to Insurer2.xls")
    ...
    Protected Sub btnDownloadEndorse_click(ByVal sender As Object, ByVal e As EventArgs)
        Dim s = DirectCast(sender, ASPxButton)
        Dim _NotificationID = s.CommandArgument.ToString()
    ...
    Protected Sub btnDownloadAmend_click(ByVal sender As Object, ByVal e As EventArgs)
        Dim s = DirectCast(sender, ASPxButton)
        Dim _NotificationID = s.CommandArgument.ToString()
    ...
    Protected Sub gridProject_HtmlRowCreated(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridViewTableRowEventArgs) Handles MyGrid.HtmlRowPrepared
        If e.RowType <> DevExpress.Web.GridViewRowType.Data Then
            Return
        End If
        Dim _NoticeID = e.KeyValue().ToString()
        Dim cmdUpload As ASPxButton = MyGrid.FindRowCellTemplateControl(e.VisibleIndex, Nothing, "cmdUpload")
        Dim cmdExportEnds As ASPxButton = MyGrid.FindRowCellTemplateControl(e.VisibleIndex, Nothing, "cmdExportEnds")
        Dim cmdSendEnds As ASPxButton = MyGrid.FindRowCellTemplateControl(e.VisibleIndex, Nothing, "cmdSendEnds")
        Dim cmdExportAmend As ASPxButton = MyGrid.FindRowCellTemplateControl(e.VisibleIndex, Nothing, "cmdExportAmend")
        Dim cmdSendAmend As ASPxButton = MyGrid.FindRowCellTemplateControl(e.VisibleIndex, Nothing, "cmdSendAmend")

        cmdUpload.JSProperties.Add("cpNoticeID", _NoticeID)
        cmdExportEnds.JSProperties.Add("cpNoticeID", _NoticeID)
        cmdExportEnds.CommandArgument = _NoticeID
        cmdSendEnds.JSProperties.Add("cpNoticeID", _NoticeID)
        cmdExportAmend.JSProperties.Add("cpNoticeID", _NoticeID)
        cmdExportAmend.CommandArgument = _NoticeID
        cmdSendAmend.JSProperties.Add("cpNoticeID", _NoticeID)
    ...
    Protected Sub cbShowUpload_Callback(ByVal source As Object, ByVal e As CallbackEventArgs) Handles cbShowUpload.Callback
        Dim _NotificationID = e.Parameter
        Session("EditNotificationID") = _NotificationID
        e.Result = _NotificationID
    End Sub



    Protected Sub cbUpload_Callback(ByVal source As Object, ByVal e As CallbackEventArgs) Handles cbUpload.Callback
        Dim _textdata = e.Parameter

        e.Result = importdata(_textdata)
    End Sub

    Protected Sub cbExportEnds_Callback(ByVal source As Object, ByVal e As CallbackEventArgs) Handles cbExportEnds.Callback
        Dim _NotificationID = e.Parameter
    ...
    Protected Sub cbSendEnds_Callback(ByVal source As Object, ByVal e As CallbackEventArgs) Handles cbSendEnds.Callback
        Dim _NotificationID = e.Parameter
    ...
    Protected Sub cbExportAmend_Callback(ByVal source As Object, ByVal e As CallbackEventArgs) Handles cbExportAmend.Callback
        Dim _NotificationID = e.Parameter
    ...
    Protected Sub cbSendAmend_Callback(ByVal source As Object, ByVal e As CallbackEventArgs) Handles cbSendAmend.Callback
        Dim _NotificationID = e.Parameter
    ...
    '====================================================================================================
    Private Function importdata(ByVal txtdata As String) As String
        ...
        Dim i As Integer = 0
        Dim reader = New StringReader(txtdata.Replace("\n", " "))
        Dim line As String
        ...
        While line IsNot Nothing
            ...
            Else
                ...
                With _itemdata
                    ...
                    .NoticeID = Session("EditNotificationID")
        ...
        Else
            Using dc As New DataClasses_CPSDataContext
                ...
                dc.ExecuteCommand("delete from tblNoticeDetail where NoticeID={0}", Session("EditNotificationID"))

                dc.tblNoticeDetails.InsertAllOnSubmit(_BillingNotificationData)

                dc.SubmitChanges()

                Result = _BillingNotificationData.Count
            End Using
    ...
    '        'Dim _data = dc_TIDMotor.SP_CLIENT_SEARCH(_clientcode).FirstOrDefault()
    '        Dim _data = (From c In dc_TIDMotor.V_MotorEndorsements Where c.REFNO.Equals(_REFNO) And c.Motor.Equals("M2")).FirstOrDefault()
    ...
    '                Dim _Ament = _AmentCode.Split(":") 'ex. 1 : แก้ไขชื่อสกุลผู้เอาประกัน กรมธรรม์
    '                .f11 = _Ament(0).ToString().Trim() 'f11 Amendment Details
    '                .f15 = _Ament(1).ToString().Trim()
    ...
    '                    '========================= 1.Endorse Only (1-22) ================
    '                    Case "1", "2", "3"
    '                        '1 : แก้ไขชื่อสกุลผู้เอาประกัน กรมธรรม์, 
    '                        '2 : แก้ไขชื่อสกุลผู้เอาประกัน พรบ., 
    '                        '3 : แก้ไขชื่อสกุลผู้เอาประกัน กรมธรรม์และพรบ.
    '                        '.f12 = _data.ClientNameOld
    '                        '.f13 = _data.ClientName
    '                        .f14 = "1"
    '                    Case "4", "5", "6"
    '                        '4 : แก้ไขที่อยู่ผู้เอาประกัน กรมธรรม์
    '                        '5 : แก้ไขที่อยู่ผู้เอาประกัน พรบ.
    '                        '6 : แก้ไขที่อยู่ผู้เอาประกัน กรมธรรม์และพรบ.
    '                        '.f12 = _data.Address1Old
    '                        '.f13 = _data.Address1
    '                        .f14 = "1"
    '                    Case "7", "9"
    '                        '7 : แก้ไขวันคุ้มครองกรมธรรม์
    '                        '9 : แก้ไขวันคุ้มครองกรมธรรม์และพรบ.
    '                        '.f12 = String.Format("{0} - {1}", _data.StartDateOld.Value.ToString("dd/MM/yyyy"), _data.EndDateOld.Value.ToString("dd/MM/yyyy"))
    '                        '.f13 = String.Format("{0} - {1}", _data.StartDate.Value.ToString("dd/MM/yyyy"), _data.EndDate.Value.ToString("dd/MM/yyyy")) '_data.StartDate.Value.ToString("dd/MM/yyyy")

    '                        .f14 = "1"
    '                    Case "8"
    '                        '8 : แก้ไขวันคุ้มครองพรบ.
    '                        'If _data.CMI_StartDateOld IsNot Nothing Then
    '                        '    .f12 = _data.CMI_StartDateOld.Value.ToString("dd/MM/yyyy")
    '                        '    .f13 = _data.CMI_StartDate.Value.ToString("dd/MM/yyyy")
    '                        .f14 = "1"
    ...
    '                        .f14 = "1"
    '                    Case "16"
    '                        '16 : แก้ไขชื่อผู้ขับขี่ คนที่ 1
    '                        '.f12 = ""
    '                        '.f13 = String.Format("{0} (อายุ {1} ปี)", _data.DriverName1, _data.DriverOld1)  'f13 รายการที่แก้ไขใหม่
    '                        .f14 = "1"
    '                    Case "17"
    '                        '17 : แก้ไขชื่อผู้ขับขี่ คนที่ 2
    '                        '.f12 = ""
    '                        '.f13 = String.Format("{0} (อายุ {1} ปี)", _data.DriverName2, _data.DriverOld2) 'f13 รายการที่แก้ไขใหม่
    '                        .f14 = "1"
    '                    Case "18", "19", "20", "21", "22", "0"
    '                        '18 : เพิ่มเติมอุปกรณ์
    '                        '19 : ขยายอาณาเขต
    '                        '20 : ขยายวันคุ้มครอง กรมธรรม์
    '                        '21 : ขยายวันคุ้มครอง พรบ.
    '                        '22 : ขยายวันคุ้มครอง กรมธรรม์และพรบ.
    '                        '0 : Free Text
    '                        .f14 = "1"

    '                        '========================= 3.Amend Only (23-26) ================
    '                    Case "23", "25"
    '                        '23 : แก้ไขชื่อสกุลผู้เอาประกัน
    '                        '25 : แก้ไขชื่อสกุลผู้เอาประกันและผู้ติดต่อ
    '                        '.f12 = _data.ClientNameOld
    '                        Dim _client = (From c In dc_SIBIS.Clients Where c.Client.Equals(_clientcode)).FirstOrDefault()
    '                        .f12 = _client.Name.Trim()
    '                        .f13 = _data.ClientName
    ...
    '                        .f14 = "3"
    '                    Case "26", "28"
    '                        '26 : แก้ไขที่อยู่ผู้เอาประกัน
    '                        '28 : แก้ไขที่อยู่ผู้เอาประกันและผู้ติดต่อ
    '                        Dim _client = (From c In dc_SIBIS.Clients Where c.Client.Equals(_clientcode)).FirstOrDefault()
    '                        .f12 = String.Format("{0} {1} {2} {3}", _client.Address1.Trim(), _client.Address2.Trim(), _client.Address3.Trim(), _client.PostCode.Trim())
    ...
    '                        .f14 = "3"
    '                    Case "29", "31"
    '                        '29 : แก้ไขเบอร์โทรผู้เอาประกัน
    '                        '31 : แก้ไขเบอร์โทรผู้เอาประกันและผู้ติดต่อ
    '                        Dim _client = (From c In dc_SIBIS.Clients Where c.Client.Equals(_clientcode)).FirstOrDefault()
    '                        .f12 = String.Format("มือถือ:{0}, เบอร์บ้าน:{1}, office:{2}", _client.MobileNo.Trim(), _client.PhoneHome.Trim(), _client.PhoneBusiness.Trim()) '_data.PhoneNumber1Old
    '                        Dim _app = (From c In dc_M2.Applications Where c.AID.Equals(_data.ID)).FirstOrDefault()
    '                        .f13 = String.Format("มือถือ:{0}, เบอร์บ้าน:{1}, office:{2}", _app.Mobile.Trim(), _app.HomePhone.Trim(), _app.OfficePhone.Trim()) '_data.PhoneNumber1Old
    ...
    Private Sub ExportData_Amend(ByVal _NotificationID As String)
        Using dc As New DataClasses_CPSExt
            ...
            '====================== data new sheet ====================
            Dim _SheetTitle As String = ""
            Using dc_Portal = New DataClasses_PortalDataContextExt()
                Dim _mail_template = (From c In dc_Portal.MailNotifications Where c.Code.Equals(_MailNotifications_Amend)).FirstOrDefault()
                _SheetTitle = _mail_template.Name
            End Using
            Dim sheet1 As ISheet = hssfworkbook.CreateSheet(_SheetTitle)
