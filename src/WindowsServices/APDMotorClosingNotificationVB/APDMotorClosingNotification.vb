...
Public Class APDMotorClosingNotification
    ...
    Private Sub RunJob()
        ...
        If JobDate >= JobStart And JobDate <= JobEnd And IO.File.Exists(logfile) = False Then
            Dim _InsurerAttachFiles As New List(Of InsurerAttachFiles)


            Using dc As New DataClasses_LWTReportsDataContext()
                '========================= M1 ========================
                Dim _data = (From c In dc.Rawdata_M1_Closing_Dailies).ToList()
                If _data.Count > 0 Then
                    Using writer As New StreamWriter(logfile, True)
                        writer.WriteLine("M1 Data : " & _data.Count.ToString() & " - " & DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"))
                        writer.Close()
                    End Using


                    Dim _Insurers = (From c In _data _
                                Group By InsurerCode = c.InsurerCode _
                                Into g = Group _
                                Select InsurerCode _
                          ).ToList()


                    For Each _InsurerCode In _Insurers
                        _InsurerAttachFiles.Add(New InsurerAttachFiles With {.FileName = SaveToExcelM1(_InsurerCode), .InsurerCode = _InsurerCode})
                    Next
                End If

                '========================= M2 ========================

                Dim _dataM2 = (From c In dc.Rawdata_M2_Closing_Dailies).ToList()
                If _dataM2.Count > 0 Then
                    Using writer As New StreamWriter(logfile, True)
                        writer.WriteLine("M2 Data : " & _dataM2.Count.ToString() & " - " & DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"))
                        writer.Close()
                    End Using
                    Dim _M2Insurers = (From c In _dataM2 _
                                Group By InsurerCode = c.InsurerCode _
                                Into g = Group _
                                Select InsurerCode _
                          ).ToList()
                    For Each _M2InsurerCode In _M2Insurers
                        _InsurerAttachFiles.Add(New InsurerAttachFiles With {.FileName = SaveToExcelM2(_M2InsurerCode), .InsurerCode = _M2InsurerCode})
                    Next
                End If
                ...
                Dim _dataTATA = (From c In dc.Rawdata_TATA_Closing_Dailies).ToList()
                If _dataTATA.Count > 0 Then
                    Using writer As New StreamWriter(logfile, True)
                        writer.WriteLine("TATA Data : " & _dataTATA.Count.ToString() & " - " & DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"))
                        writer.Close()
                    End Using

                    Dim _TATAInsurers = (From c In _dataTATA _
                                Group By InsurerCode = c.InsurerCode _
                                Into g = Group _
                                Select InsurerCode _
                          ).ToList()
                    For Each _TATAInsurerCode In _TATAInsurers
                        _InsurerAttachFiles.Add(New InsurerAttachFiles With {.FileName = SaveToExcelTATA(_TATAInsurerCode), .InsurerCode = _TATAInsurerCode})
                    Next
                End If
            ...
            If _InsurerAttachFiles.Count > 0 Then
                Dim _Insurers = (From c In _InsurerAttachFiles _
                               Group By InsurerCode = c.InsurerCode _
                               Into g = Group _
                               Select InsurerCode _
                         ).ToList()
                ...
                For Each _InsurerCode In _Insurers
                    ...
                    WH_genxls(_InsurerCode, _InsurerAttachFiles)

                Next

            Else

                Console.WriteLine("Data :" & _InsurerAttachFiles.Count.ToString())
                WriteToSysLog("Data :" & _InsurerAttachFiles.Count.ToString(), False)
            End If
    ...
    Private Function SaveToExcelM1(ByVal InsurerCode As String)



        Dim _filename = String.Format("M1_{0}_{1}.xls", InsurerCode, Now.ToString("yyyyMMddHHmmss"))
        Dim SavePath = uploadpath & "\" & _filename


        Dim viewer As New ReportViewer()
        viewer.ServerReport.ReportServerUrl = New Uri("http://lockthbnk-db07/ReportServer")
        viewer.ServerReport.ReportPath = "/Reports/rptMotorClosingDailyM1"

        Dim parameters As New List(Of ReportParameter)
        parameters.Add(New ReportParameter("InsurerCode", InsurerCode, False))
    ...
    Private Function SaveToExcelM2(ByVal InsurerCode As String)



        Dim _filename = String.Format("M2_{0}_{1}.xls", InsurerCode, Now.ToString("yyyyMMddHHmmss"))
        Dim SavePath = uploadpath & "\" & _filename


        Dim viewer As New ReportViewer()
        viewer.ServerReport.ReportServerUrl = New Uri("http://lockthbnk-db07/ReportServer")
        viewer.ServerReport.ReportPath = "/Reports/rptMotorClosingDailyM2"
    ...
    Private Function SaveToExcelTATA(ByVal InsurerCode As String)



        Dim _filename = String.Format("TATA_{0}_{1}.xls", InsurerCode, Now.ToString("yyyyMMddHHmmss"))
        Dim SavePath = uploadpath & "\" & _filename


        Dim viewer As New ReportViewer()
        viewer.ServerReport.ReportServerUrl = New Uri("http://lockthbnk-db07/ReportServer")
        viewer.ServerReport.ReportPath = "/Reports/rptMotorClosingDailyTATA"
    ...
    Private Sub WH_genxls(ByVal _InsurerCode As String, ByVal _InsurerAttachFiles As List(Of InsurerAttachFiles))


        Dim _NoticeCode As String = "B0038"
        ...
        Using dc_portal = New DataClasses_PortalDataContext()
            ...
            Using dc_CPS As New DataClasses_CPSDataContext()
                Dim _Insurer = (From c In dc_CPS.tblNoticeMailContacts Where c.Code.Equals(_InsurerCode) And c.NoticeCode.Equals(_NoticeCode)).FirstOrDefault()

                Dim _ClosingDate = Today.AddDays(-1).ToString("dd.MM.yyyy")
                ...
                '==============Add an Attachment=========================
                Dim _AttachFiles = (From c In _InsurerAttachFiles Where c.InsurerCode = _InsurerCode Order By c.FileName).ToList()
                For Each item In _AttachFiles
                    Dim _AttachFile = New Attachment(item.FileName, "application/vnd.ms-excel")

                    If item.FileName.IndexOf("M1") > -1 Then
                        _AttachFile.Name = String.Format("{0} - Motor1.xls", _Insurer.Name)
                    ElseIf item.FileName.IndexOf("M2") > -1 Then
                        _AttachFile.Name = String.Format("{0} - Motor2.xls", _Insurer.Name)
                    ElseIf item.FileName.IndexOf("TATA") > -1 Then
                        _AttachFile.Name = String.Format("{0} - TATA.xls", _Insurer.Name)
                    End If


                    _AttachFile.TransferEncoding = System.Net.Mime.TransferEncoding.QuotedPrintable
                    msg.Attachments.Add(_AttachFile)
                Next
...
<Serializable()> _
Public Class InsurerAttachFiles
